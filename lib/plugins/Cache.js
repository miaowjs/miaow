...
Cache.prototype.inject = function(module, taskContext, compilation, callback) {
  ...
  function inject(module, callback) {
    ...
    detect(function(cachedModule) {
      compilation.applyPluginsParallelBailResult(
        'inject-cache', module, cachedModule, function(err, reject) {
        if (!err && !reject && cachedModule) {
          // 只保留创建模块和属性拷贝任务
          module.tasks = [
            {
              task: emitModuleTask,
              options: {
                modules: cachedModule.emitModules.map(function(src) {
                  var emitModule = cache.emitModules[src];

                  return {
                    src: src,
                    contents: emitModule.srcContents || emitModule.contents
                  };
                })
              }
            },
