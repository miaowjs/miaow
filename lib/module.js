...
Module.prototype.compile = function () {
  ...
  this.__compilePromise__ = Q.Promise(function (fullfill, reject) {
    if (module.verbase) {
      mutil.log('开始编译 ' + chalk.green.underline.bold(module.srcPath));
    }
    ...
    async.eachSeries([
      ...
      if (module.verbase) {
        mutil.log('成功编译 ' + chalk.green.underline.bold(module.srcPath));
      }
...
Module.prototype.destroy = function () {
  ...
  // 删除依赖关系
  this.dependList.forEach(function (dependSrcPath) {
    var dependModule = cache.get(dependSrcPath);
    dependModule && module.unDepend(dependModule);
  });

  // 删除被依赖关系
  this.dependedList.forEach(function (dependedSrcPath) {
    var dependedModule = cache.get(dependedSrcPath);
    dependedModule && dependedModule.unDepend(module);
  });
...
Module.prototype.unDepend = function (module) {
  _.remove(this.dependList, module.srcPath);
  _.remove(module.dependedList, this.srcPath);
};
...
Object.defineProperty(Module.prototype, 'verbase', {
  get: function () {
    return this.options.verbase;
  },
  set: function (verbase) {
    this.options.verbase = verbase;
  }
});
