...
function Module(srcPath, config, cache, contents) {
  ...
  function getModuleOption(config, path, relativeFilePath) {
    return _.find(_.result(config, path) || [], function (item) {
        var testList = _.isArray(item.test) ? item.test : [item.test];
        return _.any(testList, function (test) {
          if (_.isRegExp(test)) {
            return test.test(relativeFilePath);
          } else {
            return minimatch(relativeFilePath, test, {dot: true});
          }
        });
      }) || {};
  }
...
Module.prototype.createModule = function (relativePath, contents, cb) {
  ...
  var module = new Module(srcPath, this.config, this.cache, contents);
...
// 是否主入口
Object.defineProperty(Module.prototype, 'isMain', {
  get: function () {
    this.__isMain__ = this.__isMain__ || mutil.isMain(this.srcAbsPath, this.cwd);
    return this.__isMain__;
  },
  set: function (isMain) {
    this.__isMain__ = isMain;
  }
});

// 是否可以被打包进其他模块
Object.defineProperty(Module.prototype, 'canBePacked', {
  get: function () {
    return !(this.isMain || this.packed);
  }
});
