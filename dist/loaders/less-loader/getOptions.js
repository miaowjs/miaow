...
var urlVisitor = require('./urlVisitor');
...
function getOptions(loaderContext) {
  ...
  // 增加 url 引用的处理
  // 将 LESS 文件中的 url(foo) 转换成 url(~foo)
  options.plugins.push(urlVisitor);
  // 将 CSS 中的 url(foo) 转换成 url(./foo)
  options.plugins.push(urlProcessor);
